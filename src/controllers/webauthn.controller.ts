...
// El dominio donde vive tu frontend (Harvein usará localhost por ahora)
...
export const generateRegistration = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    // El servidor genera un desafío criptográfico
...
export const verifyRegistration = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (verification.verified && verification.registrationInfo) {
      ...
      // Guardamos la huella/FaceID en la bóveda
      ...
      // Limpiamos el desafío por seguridad
      await prisma.user.update({
        where: { id: user.id },
